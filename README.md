...
## PW / Next Topper dashboards

`PW ULTIMATE` and `NEXT TOPPER ULTIMATE` each open their own internal page (`/pw`, `/next-topper`) listing that group's platforms. Tapping one of those opens the real site **directly in a new browser tab** (`target="_blank"`), the same way the other direct-link cards (Vibrant Academy, PW PI PRO) already work.

This used to route through an in-app iframe viewer so the real URL never showed in the address bar — that's been removed. Some sites would refuse to load inside an iframe at all (their own `X-Frame-Options`/CSP headers), so it was unreliable; opening directly in the browser always works.
