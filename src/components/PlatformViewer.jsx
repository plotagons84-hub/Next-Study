...
export default function PlatformViewer({ platform, backTo }) {
  ...
  // Guards a direct/bookmarked link to a platform that's since been locked
  // (or never had a URL set) - the dashboard already hides these from being
  // clicked, but a saved URL could still land here directly.
  if (platform.locked || !platform.url) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center px-6">
        <div className="h-14 w-14 rounded-full bg-white/5 border border-white/10 grid place-items-center">
          <Lock size={22} className="text-white/40" />
        </div>
        <p className="text-white font-display font-semibold mt-4">{platform.name} is locked</p>
        <p className="text-sm text-white/50 mt-1">Check back soon.</p>
        <Link to={backTo} className="mt-5 text-orange-400 hover:text-orange-300 text-sm">
          &larr; Back
        </Link>
      </div>
    )
